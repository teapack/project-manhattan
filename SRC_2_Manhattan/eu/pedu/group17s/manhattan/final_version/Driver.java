...
public class Driver implements IDriver {
    ...
    public Driver(IVehicle vehicle) {
        ...
        planner = Holder.getFactories().newPlanner();
    }
    
    public Driver(IVehicle vehicle, boolean specialPlanner) {
        this.vehicle = vehicle;
        planner = ((Factory)Holder.getFactories()).newSpecialPlanner();
    }
    ...
    public void nextStep() {
        ...
        if (destination == null) {
            destination = planner.suggestNextDestination(vehicle);
...
//                        + " narazilo do budovy.");
    ...
    private boolean equalDestination(IStreet street, Destination destination) {
        if (street.getPosition().x == destination.destStreet.getPosition().x
                && street.getPosition().y == destination.destStreet
                .getPosition().y) {
            if (((Vehicle) this.vehicle).isSpecialCar()) {
                if (specialLine != ((Vehicle) this.vehicle).getSpecialLine()) {
                    this.specialLine = ((Vehicle) this.vehicle).getSpecialLine();
                }
                specialLine++;
                ((Vehicle) this.vehicle).setSpecialLine(specialLine);
            }
            return true;
        }
        return false;
