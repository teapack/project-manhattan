...
public class Factory implements IFactories {
    ...
    public IVehicle newSpecialVehicle(Position position, Direction8 direction) {
        IVehicle vehicle = new Vehicle(position, direction);
        newSpecialDriver(vehicle);
        if (townCreated) {
            Clock.getInstance().addListener((IListener) vehicle);
            GUI.getCanvasManager().add(vehicle);
        }
    ...
    public IDriver newSpecialDriver(IVehicle vehicle) {
        IDriver driver = new Driver(vehicle, true);
        Town.getInstance().addDriver(driver);
        return driver;
    }
    ...
    public IPlanner newSpecialPlanner() {
        return new SpecialPlanner();
    }
